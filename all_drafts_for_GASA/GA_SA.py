    ...
    for i in range(iterL):
        ...
        while flag:
            ...
            if max(genetic_population_new) >1200 or min(genetic_population_new)<=0:
                ...
                if S11 < -15 and S22 < -15 and value > 1:
...
print("增益：",function(OUTCOME[GAIN.index(max(GAIN))])[0])
print("S11,S22：",function(OUTCOME[GAIN.index(max(GAIN))])[1],function(OUTCOME[GAIN.index(max(GAIN))])[2],"\n")
...
plt.xlabel('GA迭代次数', fontproperties=my_font)
plt.ylabel('GA最优解', fontproperties=my_font)

# GA平均适应度随迭代次数的变化
fig2 = plt.figure(2)
plt.plot(range(1, generations + 1), fitness_mean)
plt.xlabel('GA迭代次数', fontproperties=my_font)
plt.ylabel('GA平均适应度', fontproperties=my_font)
plt.show()
"""
